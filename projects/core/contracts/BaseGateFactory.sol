...
contract BaseGateFactory is IBaseGateFactory {
    ...
    function createPool(address tokenA, address tokenB, uint24 fee) external override returns (address pool) {
        ...
        pool = IBaseGatePoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IBaseGatePool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IBaseGatePool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IBaseGatePool(pool).setLmPool(lmPool);
    }
}

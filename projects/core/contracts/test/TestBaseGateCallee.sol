...
import '../interfaces/callback/IBaseGateMintCallback.sol';
...
contract TestBaseGateCallee is IBaseGateMintCallback, IBaseGateSwapCallback, IBaseGateFlashCallback {
    ...
    function swapToLowerSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IBaseGatePool(pool).swap(recipient, true, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }

    function swapToHigherSqrtPrice(address pool, uint160 sqrtPriceX96, address recipient) external {
        IBaseGatePool(pool).swap(recipient, false, type(int256).max, sqrtPriceX96, abi.encode(msg.sender));
    }
    ...
    function mint(address pool, address recipient, int24 tickLower, int24 tickUpper, uint128 amount) external {
        IBaseGatePool(pool).mint(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }

    event MintCallback(uint256 amount0Owed, uint256 amount1Owed);

    function baseGateMintCallback(uint256 amount0Owed, uint256 amount1Owed, bytes calldata data) external override {
        address sender = abi.decode(data, (address));

        emit MintCallback(amount0Owed, amount1Owed);
        if (amount0Owed > 0)
            IERC20Minimal(IBaseGatePool(msg.sender).token0()).transferFrom(sender, msg.sender, amount0Owed);
        if (amount1Owed > 0)
            IERC20Minimal(IBaseGatePool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
    }
    ...
    function flash(
        ...
        IBaseGatePool(pool).flash(recipient, amount0, amount1, abi.encode(msg.sender, pay0, pay1));
    }

    function baseGateFlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external override {
        emit FlashCallback(fee0, fee1);

        (address sender, uint256 pay0, uint256 pay1) = abi.decode(data, (address, uint256, uint256));

        if (pay0 > 0) IERC20Minimal(IBaseGatePool(msg.sender).token0()).transferFrom(sender, msg.sender, pay0);
        if (pay1 > 0) IERC20Minimal(IBaseGatePool(msg.sender).token1()).transferFrom(sender, msg.sender, pay1);
    }
}

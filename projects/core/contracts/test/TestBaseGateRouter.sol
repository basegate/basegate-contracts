...
contract TestBaseGateRouter is IBaseGateSwapCallback {
    ...
    function baseGateSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) public override {
        ...
        if (pools.length == 1) {
            ...
            address tokenToBePaid = amount0Delta > 0
                ? IBaseGatePool(msg.sender).token0()
                : IBaseGatePool(msg.sender).token1();
            int256 amountToBePaid = amount0Delta > 0 ? amount0Delta : amount1Delta;

            bool zeroForOne = tokenToBePaid == IBaseGatePool(pools[0]).token1();
            IBaseGatePool(pools[0]).swap(
                msg.sender,
                zeroForOne,
                -amountToBePaid,
            ...
            if (amount0Delta > 0) {
                IERC20Minimal(IBaseGatePool(msg.sender).token0()).transferFrom(
                    payer,
                    msg.sender,
                    uint256(amount0Delta)
                );
            } else {
                IERC20Minimal(IBaseGatePool(msg.sender).token1()).transferFrom(
                    payer,
                    msg.sender,

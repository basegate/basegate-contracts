...
describe('BaseGateFactory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: BaseGatePoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as BaseGatePoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as BaseGateFactory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [BaseGateFactory, BaseGatePoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('BaseGatePool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('BaseGatePool')

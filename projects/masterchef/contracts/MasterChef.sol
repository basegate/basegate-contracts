...
contract MasterChef is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    /// @notice baseGatePoolPid[token0][token1][fee] => pid
    mapping(address => mapping(address => mapping(uint24 => uint256))) baseGatePoolPid;
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IBaseGatePool indexed baseGatePool, ILMPool indexed lmPool);
    ...
    function pendingCake(uint256 _tokenId) external view returns (uint256 reward) {
        ...
        if (positionInfo.pid != 0) {
            ...
            ILMPool LMPool = ILMPool(pool.baseGatePool.lmPool());
    ...
    function add(uint256 _allocPoint, IBaseGatePool _baseGatePool, bool _withUpdate) external onlyOwner {
        if (_withUpdate) massUpdatePools();

        ILMPool lmPool = LMPoolDeployer.deploy(_baseGatePool);

        totalAllocPoint += _allocPoint;
        address token0 = _baseGatePool.token0();
        address token1 = _baseGatePool.token1();
        uint24 fee = _baseGatePool.fee();
        if (baseGatePoolPid[token0][token1][fee] != 0) revert DuplicatedPool(baseGatePoolPid[token0][token1][fee]);
        ...
        baseGatePoolPid[token0][token1][fee] = poolLength;
        baseGatePoolAddressPid[address(_baseGatePool)] = poolLength;
    ...
    function onERC721Received(
        ...
        uint256 pid = baseGatePoolPid[cache.token0][cache.token1][cache.fee];

import { abi as IBaseGatePoolABI } from '@basegate_io/core/artifacts/contracts/interfaces/IBaseGatePool.sol/IBaseGatePool.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, ContractTransaction, Wallet } from 'ethers'
import { ethers, waffle } from 'hardhat'
import { IBaseGatePool, IWETH9, MockTimeSwapRouter, TestERC20 } from '../typechain-types'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IBaseGatePool, IBaseGatePool, IBaseGatePool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IBaseGatePoolABI, wallet)) as [
      IBaseGatePool,
      IBaseGatePool,
      IBaseGatePool
    ]
  ...
  let pools: [IBaseGatePool, IBaseGatePool, IBaseGatePool]
  ...
  describe('#exactInput', () => {
    ...
    it.skip('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestBaseGateCallee')
